...
**Same-repository PRs** (including Renovate): `build-candidates` builds all six images once per revision, pushes them to GHCR tagged `pr-<number>-<head-sha>` (immutable — a new commit gets a new tag rather than overwriting the previous revision's images), and captures each image's manifest digest as a job output. `security-scan`, `functional-amd64`, `functional-arm64`, `component-generic`, `component-sniproxy`, `component-repo-compose`, and `artifact-contracts` all consume those exact digests, so updating a PR cannot cause a test to run against a stale image.

`security-scan` (`.github/workflows/security-scan.yml`, reusable) runs the identical Trivy scan and HIGH/CRITICAL-with-fix-available threshold that `release.yml` uses — see "Release" below for how it works. Running it here too, not just after merge, means a vulnerability newly disclosed against an otherwise-unchanged base image (the reason PR #68/#69/#70 didn't catch the `libudev1` CVEs that later failed a `main` build: `security-scan` used to exist only in `release.yml`, which never runs on `pull_request`) now fails the PR itself.

**External fork PRs**: GitHub always issues a read-only `GITHUB_TOKEN` for `pull_request` runs from forks, regardless of the `permissions:` declared in the workflow, so `build-candidates` cannot push to GHCR for fork PRs. `fork-build-test` runs instead: a local, AMD64-only build (`--load`, no `--push`, no registry login) with a basic heartbeat smoke test. Fork PRs do not get ARM64, artifact-contract, or security-scan coverage (the latter needs `security-events: write`, which forks don't receive either).
...
2. **`build-candidates`**: the same reusable workflow `pr-ci.yml` calls, given the exact resolved SHAs above (not a floating branch), tagged `candidate-<run-id>`. Base-image `FROM` lines are rewritten by `scripts/pin-base-image.sh`, which fails the build if its expected line doesn't appear in the target Dockerfile exactly once, rather than silently doing nothing (or the wrong thing) on a zero- or multi-match. This also overrides `lancache-dns`'s `dnstool` builder stage (`DNS_GO_BUILDER_IMAGE`), since upstream's Dockerfile pins an old `golang:1.23.1-alpine` with unpatched stdlib CVEs that upstream hasn't bumped — tracked as its own Renovate `customManager` in `renovate.json` so it updates the same way the Trivy CLI version does (see below).

   The `lancachenet/ubuntu` build step additionally patches its own upstream Dockerfile with a date-based `CACHEBUST` build-arg, wired into the command text of the `apt-get upgrade` layer upstream already runs right after `FROM`. Without it, that layer is a single cached step: with an unchanged Dockerfile and resolved SHA, `--cache-from` replays whatever OS packages were current the *first* time it built, not what's current now — so a newly published, fix-available CVE against an *unchanged* base image would sit unpatched indefinitely. `CACHEBUST` forces that layer to re-run once per day, independent of whether `lancachenet/ubuntu` itself ever changes. This is the general pattern for patching any infrequently-updated upstream base image: bust the cache on its own patch/upgrade layer rather than waiting on upstream to cut a new commit.
...
4. **`security-scan`**: reusable workflow (`.github/workflows/security-scan.yml`, also called by `pr-ci.yml` — see "PR CI" above). `aquasecurity/trivy-action` scans each exact candidate digest (HIGH/CRITICAL, `os,library`), one image per step so a failure in one doesn't stop the others from being scanned. `ignore-unfixed: true` means only vulnerabilities with an available fix can block — this is what makes blocking safe: there's no way for pre-existing, unpatched debt to permanently wedge every release, since anything without a fix is simply not counted. Each image's SARIF results are uploaded to the repository's Security tab (`github/codeql-action/upload-sarif`, one category per image) regardless of outcome, so findings are visible between releases too, not just in the Actions log. This mirrors the pattern already proven in `oct8l/apt-cacher-ng`, including its exact `trivy-action`/`codeql-action` pins.

   `limit-severities-for-sarif: true` is required alongside `severity: 'HIGH,CRITICAL'`: `trivy-action`'s entrypoint unsets `TRIVY_SEVERITY` whenever `format: sarif` unless this is explicitly set, so without it the job silently gates on *every* severity (down to `UNKNOWN`/`LOW`), not just HIGH/CRITICAL as configured — the root cause of a `main` build failing on two `MEDIUM` `libudev1` findings (`CVE-2026-16742`, `CVE-2026-15059`) that the configured threshold should have excluded.
...
- Review the finding in the repository's Security tab (one category per image) for the actual CVE, package, and fixed version — the job's own log only reports pass/fail per image, not the finding detail.
- If the reported severity looks lower than `HIGH`/`CRITICAL` (the configured threshold), don't assume the gate is misbehaving on its own — `limit-severities-for-sarif: true` in `security-scan.yml` is what keeps the threshold honest; see the "Release" section above.
- If the same base image was clean on a previous run and nothing in this repo's Dockerfiles or pinned refs changed, it's very likely a newly disclosed CVE, not a regression — `build-candidates.yml`'s `CACHEBUST` mechanism (see "Release" above) ensures the next build actually re-patches rather than replaying a stale cached layer.
